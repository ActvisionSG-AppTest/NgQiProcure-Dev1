    ...
    public class ProfileAppService : QiProcureDemoAppServiceBase, IProfileAppService
    {
        ...
        public async Task<CurrentUserProfileEditDto> GetCurrentUserProfileForEdit()
        {
            ...
            userProfileEditDto.QrCodeSetupImageUrl = user.GoogleAuthenticatorKey != null
                ? _googleTwoFactorAuthenticateService.GenerateSetupCode("QiProcureDemo",
        ...
        public async Task<String> GetProfilePictureByIdString(Guid profilePictureId)
        {
            var bytes = await GetProfilePictureByIdOrNull(profilePictureId);
            if (bytes == null)
            {
