...
namespace QiProcureDemo.Web.Url
{
    public abstract class AppUrlServiceBase : IAppUrlService, ITransientDependency
    {
        ...
        public abstract string ApprovalRoute { get; }													 
        ...
        public string CreateApprovalUrlFormat(string tenancyName)
        {
            var approvalLink = WebUrlService.GetSiteRootAddress(tenancyName).EnsureEndsWith('/') + ApprovalRoute + "?moduleName={module}&userId={userId}";

            if (tenancyName != null)
            {
                approvalLink = approvalLink + "&tenantId={tenantId}";
            }
